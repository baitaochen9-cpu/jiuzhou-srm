...
public class PfUtilBaseTools {
	...
	// 预算开发部的需求：单据审批通过后，需要调整后再次提交走一遍流程
	// 提交单据时，强制删除从前的流程实例，重新走流程实例
...
/**
	 ...
	 * @return
	 */
	public static UserDefineFunction[] changeFunctionVOs(
			ArrayList<FunctionVO> alFuncs) {
		...
		try {
			for (FunctionVO fVO : alFuncs) {
				...
				if ((fVO.getArguments() != null)
						&& (fVO.getArguments().indexOf(".") > 0)) // 表示不是函数
					continue;
				...
				if (par != null) {
					...
					for (int j = 0; j < par.length; j++) {
						classes[j] = parseTypeClass(par[j].substring(par[j]
								.indexOf(":") + 1));
	...
	public static AggregatedValueObject[] createArrayWithBilltype(
			String destBillOrTranstype, int size) throws BusinessException {
		// 得到目的单据的聚合VO类名称
		...
		if (bs instanceof AggVOStyle) {
			...
			throw new PFBusinessException(NCLangRes4VoTransl.getNCLangRes()
					.getStrByID("busitype", "busitypehint-000009")/* 单据实体必须符合聚合VO的样式 */);

		AggregatedValueObject[] retDestVOs;
		try {
			retDestVOs = (AggregatedValueObject[]) Array.newInstance(
					Class.forName(destBillVoClzName), size);
		} catch (Exception e) {
			Logger.error(e.getMessage(), e);
			throw new PFBusinessException(NCLangRes4VoTransl.getNCLangRes()
					.getStrByID("busitype", "busitypehint-000034", null,
	 ...
	 *            动作处理返回值
	 */
	public static void fetchBillId(PfParameterVO paravo,
			Object singleBillEntity, Object retObj) {
		// 从动作脚本执行后的返回值中获得单据Id
	...
	public static Object[] composeResultAry(Object tmpRet, int length, int i,
			Object[] ret) {
		if (ret == null) {
			if (tmpRet != null) {
				if (tmpRet.getClass().getName().startsWith("[")
						&& ((Object[]) tmpRet).length > 0) {
					ret = (Object[]) Array.newInstance(
							((Object[]) tmpRet)[0].getClass(), length);
				} else {
					ret = (Object[]) Array.newInstance(tmpRet.getClass(),
							length);
				}
			}
		}
		if (ret != null) {
			if (tmpRet != null&&tmpRet.getClass().getName().startsWith("[")&&((Object[]) tmpRet).length > 0)
				ret[i] = tmpRet.getClass().getName().startsWith("[") ? ((Object[]) tmpRet)[0]
						: tmpRet;
				else if(tmpRet != null){
					ret[i] =tmpRet;
				}
	 ...
	 * @return 查询到的值数组
	 */
	public static HashMap<String, String> fetchValuesByPKs(String tableName,
			String pkFieldName, String columnName, HashSet<String> hsPK) {
		String sql = "select " + pkFieldName + "," + columnName + " from "
				+ tableName;
		...
		sql += where.substring(0, where.length() - 1) + ") order by "
				+ pkFieldName;
		...
		try {
			...
			for (Object[] objs : lResult) {
				hmRet.put(objs[0].toString(),
						objs[1] == null ? null : objs[1].toString());
			}
	...
	/**
	 * 查询某单据类型的父单据类型 <li>根据单据大类bd_billtype.billstyle
	 * 
	 * @param btVO
	 *            单据类型VO对象
	 * @return
	...
	private static String findParentBilltypeByStyle(BilltypeVO btVO) {
		if (btVO.getBillstyle() == null)
			return null;
		String parentBilltype = PfDataCache.getBillTypeByStyle(btVO
				.getBillstyle().toString());
		if (parentBilltype == null
				|| parentBilltype.equals(btVO.getPk_billtypecode()))
			// 单据大类不存在 或本身就是父单据类型
			return null;
	...
	public static BasedocVO[] getAllBasedocVO() {
		...
		for (int i = 0; i < vos.length; i++) {
			...
			vos[i].setDocCode(classVOs[i].getName());
	...
	private static ClassVO[] queryAllRefInfoVOs() throws BusinessException {
		return (ClassVO[]) NCLocator
				.getInstance()
				.lookup(IUAPQueryBS.class)
				.retrieveByClause(
						ClassVO.class,
						"help like '%DOC%' and classtype=" + IType.ENTITY
								+ " and refmodelname is not null")
				.toArray(new ClassVO[0]);
	}

	/**
	 * 从动作脚本执行后的返回值中获得单据Id <li>返回值必须为ArrayList或String类型
	 * 
	 * @param tmpObj
	 */
	private static String getBillID(Object tmpObj) {
		Logger.debug(NCLangRes4VoTransl.getNCLangRes().getStrByID("busitype",
				"busitypehint-000037")/***** 从动作脚本执行后的返回值中获得单据Id开始 *****/
		);
		String billId = null;
		if (tmpObj == null) {
			return billId;
	...
	/**
	 * 从单据实体中获取平台相关信息 <li>根据单据实体的元数据模型
	 * 
	 * @param headvo
	 * @param singleBillEntity
	 *            单据实体对象，非数组
	 ...
	 * @since 5.5
	 */
	public static int getHeadInfoByMeta(PublicHeadVO headvo,
			Object singleBillEntity, String billType) {
		headvo.billType = billType;

		IFlowBizItf fbi = PfMetadataTools.getBizItfImpl(singleBillEntity,
				IFlowBizItf.class);
		if (fbi == null)
			throw new PFRuntimeException(NCLangRes4VoTransl.getNCLangRes()
					.getStrByID("busitype", "busitypehint-000038")/* 元数据实体没有提供业务接口IFlowBizItf的实现类 */);
	...
	public static String getRealBilltype(String strTypeCodestr) {
		...
		if (btVO == null)
			throw new PFRuntimeException(NCLangRes4VoTransl.getNCLangRes()
					.getStrByID("busitype", "PfUtilBaseTools-000000", null,
							new String[] { billtype })/* 错误：流程平台缓存中不存在该单据或交易类型={0} */);
		else if (btVO.getIstransaction() != null
	...
	public static String getRealBilltypeByPK(String strPK) {
		Collection<BilltypeVO> collbilltypes = PfDataCache.getBilltypes()
				.values();
		for (BilltypeVO vo : collbilltypes) {
			if (vo != null && vo.getPrimaryKey().equals(strPK)) {
				if (vo.getIstransaction() != null
						&& vo.getIstransaction().booleanValue()) {
		...
		throw new PFRuntimeException(NCLangRes4VoTransl.getNCLangRes()
				.getStrByID("busitype", "PfUtilBaseTools-000000", null,
						new String[] { strPK })/* 错误：流程平台缓存中不存在该单据或交易类型={0} */);
	}
	/**
	 * 根据单据or交易类型编码返回其名称
	 * @param billType
	...
	public static String[] getStrBillVo(String pkBillType)
		...
		// 取得主实体所有1:n组合的实体
		List<IBean> lRelatedBeans = be.getRelatedEntities(
				AssociationKind.Composite, ICardinality.ASS_ONE2MANY);

		// FIXME:取第一个组合的实体？
		IBean relatedBean = lRelatedBeans.size() > 0 ? lRelatedBeans.iterator()
				.next() : null;
		retStrs[2] = relatedBean == null ? null : relatedBean
				.getFullClassName();
	 ...
	 * @throws BusinessException
	 */
	public static PfParameterVO getVariableValue(String billType,
			String actionName, AggregatedValueObject billvo,
			AggregatedValueObject[] billvos, Object userObj, Object[] userObjs,
			WorkflownoteVO worknoteVO, HashMap hmParam,
			Hashtable hashBilltypeToParavo) throws BusinessException {
		// 平台日志
		Logger.debug(">>>getVariableValue(" + actionName + "," + billType
				+ ") START<<<");
		...
		if (actionName != null && actionName.length() > 20) {
			paraVo.m_actionName = actionName.substring(0,
					actionName.length() - 20);
		...
		// 平台日志
		Logger.debug(">>>billType=" + paraVo.m_billType + " busiType="
				+ paraVo.m_businessType);
		Logger.debug(">>>billMaker=" + paraVo.m_makeBillOperator + " operator="
				+ paraVo.m_operator);
		Logger.debug(">>>corpPK=" + paraVo.m_pkOrg + " billId="
				+ paraVo.m_billVersionPK);
		Logger.debug(">>>actionName=" + paraVo.m_actionName + " billNo="
				+ paraVo.m_billNo);
		// 设置单据类型-工作流参数VO
		if (hashBilltypeToParavo != null) {
			String src_billtypePK = StringUtil
					.isEmptyWithTrim(paraVo.m_preValueVo.getParentVO()
							.getPrimaryKey()) ? "" : paraVo.m_preValueVo
					.getParentVO().getPrimaryKey();
			hashBilltypeToParavo
					.put(paraVo.m_billType + src_billtypePK, paraVo);
		}
	...
	public static PfParameterVO getVariableValue(String billType,
			...
			AggregatedValueObject[] billvos, Object userObj, Object[] userObjs,
			WorkflownoteVO worknoteVO, HashMap hmParam,
			Hashtable hashBilltypeToParavo, String src_billtyePK) {
		// 平台日志
		Logger.debug(">>>getVariableValue(" + actionName + "," + billType
				+ ") START<<<");
	...
	/**
	 * 判断某单据动作编码是否为"审批"动作 <li>即以"APPROVE"开头
	 * 
	 * @param actionName
	 * @return
	 */
	public static boolean isApproveAction(String actionName, String billtype) {
		...
		if (actionVo == null || actionVo.getAction_type() == null) {
			...
			return actionName.length() >= leng
					&& actionName.toUpperCase().substring(0, leng)
							.equals(IPFActionName.APPROVE);
		} else {
			return actionVo.getAction_type() == BillActionTypeEnum.SignalApproveflow
					.getIntValue();
		}
	}

	private static BillactionVO getBillActionVO(String actionName,
			String billtype) {
		Map<String, BillactionVO> actionMap = PfDataCache
				.getBillactionVOMap(billtype);
	...
	public static boolean isDeleteAction(String actionName, String billtype) {
		...
		BillactionVO actionVo = getBillActionVO(actionName, billtype);
		if (actionVo == null || actionVo.getAction_type() == null) {
			return actionName.endsWith(IPFActionName.DEL_DELETE)
					|| actionName.endsWith(IPFActionName.DEL_DISCARD)
					|| actionName.endsWith(IPFActionName.DEL_SOBLANKOUT);
		} else {
			return actionVo.getAction_type() == BillActionTypeEnum.Delete
					.getIntValue();
		}
	}

	/**
	 * 判断某单据动作编码是否为"回退"动作 <li>是否以ROLLBACK开头
	 * 
	 * @param actionName
	...
	public static boolean isRollbackAction(String actionName, String billtype) {
		...
		if (actionVo == null || actionVo.getAction_type() == null) {
			...
			return actionName.length() >= leng
					&& actionName.toUpperCase().substring(0, leng)
							.equals(IPFActionName.ROLLBACK);
		} else {
			return actionVo.getAction_type() == BillActionTypeEnum.RollbackWorkflow
					.getIntValue();
		}
	...
	/**
	 * 判断某单据动作编码是否为"提交"或"编辑"动作 <li>即以"SAVE"或"EDIT"开头
	 * 
	 * @param actionName
	 *            动作编码
	 * @return
	...
	public static boolean isSaveAction(String actionName, String billtype) {
		...
		if (actionVo == null || actionVo.getAction_type() == null) {
			...
			return actionName.length() >= leng
					&& (actionName.toUpperCase().substring(0, leng)
							.equals(IPFActionName.SAVE) || actionName
							.toUpperCase().substring(0, leng)
							.equals(IPFActionName.EDIT));
		} else {
			return actionVo.getAction_type() == BillActionTypeEnum.StartApproveflow
					.getIntValue();
		}
	...
	/**
	 * 判断某单据动作编码是否为"执行工作流"动作 <li>是否以SIGNAL开头
	 * 
	 * @param actionName
	 *            动作编码
	 * @return
	...
	public static boolean isSignalAction(String actionName, String billtype) {
		...
		if (actionVo == null || actionVo.getAction_type() == null) {
			...
			return actionVo.getAction_type() == BillActionTypeEnum.SignalWorkflow
					.getIntValue();
		}
	}

	/**
	 * 判断某单据动作编码是否为"启动工作流"动作 <li>是否以START结尾
	 * 
	 * @param actionName
	 *            动作编码
	 * @return
	...
	public static boolean isStartAction(String actionName, String billtype) {
		...
		if (actionVo == null || actionVo.getAction_type() == null) {
			...
			return actionName.length() >= leng
					&& actionName.toUpperCase().substring(0, leng)
							.equals(IPFActionName.START);
		} else {
			return actionVo.getAction_type() == BillActionTypeEnum.StartWorkflow
					.getIntValue();
		}
	...
	public static boolean isTranstype(String strTypeCode) {
		...
		BilltypeVO btVO = PfDataCache.getBillTypeInfo(new BillTypeCacheKey()
				.buildBilltype(strTypeCode).buildPkGroup(
						InvocationInfoProxy.getInstance().getGroupId()));
	...
	/**
	 * 判断某单据动作编码是否为"弃审"动作 <li>是否以UNAPPROVE开头
	 * 
	 * @param actionName
	 *            动作编码，见<code>IPFActionName</code>
	 * @return
	...
	public static boolean isUnapproveAction(String actionName, String billtype) {
		...
		if (actionVo == null || actionVo.getAction_type() == null) {
			...
			return actionName.length() >= leng
					&& actionName.toUpperCase().substring(0, leng)
							.equals(IPFActionName.UNAPPROVE);
		} else {
			return actionVo.getAction_type() == BillActionTypeEnum.RollbackApproveflow
					.getIntValue();
		}
	}
	 ...
	 * @throws BusinessException
	 */
	public static AggregatedValueObject[] pfInitVos(String strBillVoClassName,
			int voLen) throws BusinessException {
		AggregatedValueObject[] retVos;
		try {
			retVos = (AggregatedValueObject[]) java.lang.reflect.Array
					.newInstance(Class.forName(strBillVoClassName), voLen);
		} catch (Exception e) {
	...
	public static String queryAllCheckers(String billId, String billType) {
		...
		try {
			WorkflownoteVO[] noteVOs = NCLocator
					.getInstance()
					.lookup(IPFWorkflowQry.class)
					.queryWorkitems(billId, billType,
			...
			Logger.debug(NCLangRes4VoTransl.getNCLangRes().getStrByID(
					"busitype", "busitypehint-000039", null,
					new String[] { billId, strAllCheckers.toString() })/*
																		 * >>获得单据ID
																		 * {
																		 * 0}的所有审批人
																		 * ={1}
	...
	/**
	 * 查询单据类型（或交易类型）的同源单据类型 <li>以倒序方式返回
	 * 
	 * @param billType
	 *            单据类型（或交易类型）
	 * @return
	...
	public static HashSet<String> querySimilarTypes(String billType) {
		...
		hsRet.add(billType);
		BilltypeVO btVO = PfDataCache.getBillTypeInfo(new BillTypeCacheKey()
				.buildBilltype(billType).buildPkGroup(
						InvocationInfoProxy.getInstance().getGroupId()));
		if (btVO.getIstransaction() != null
				&& btVO.getIstransaction().booleanValue()) {
			// 为交易类型，则找到其所属的单据类型
			BilltypeVO parentTypeVO = PfDataCache
					.getBillTypeInfo(new BillTypeCacheKey().buildBilltype(
							btVO.getParentbilltype()).buildPkGroup(
							InvocationInfoProxy.getInstance().getGroupId()));
	...
	/**
	 * 判断某单据动作编码是否为"取消提交"动作 <li>是否以RECALL开头
	 * 
	 * @param actionName
	 *            动作编码，见<code>IPFActionName</code>
	 * @return
	...
	public static boolean isRecallAction(String actionName, String billtype) {
		...
		if (actionVo == null || actionVo.getAction_type() == null) {
			...
			return actionName.length() >= leng
					&& actionName.toUpperCase().substring(0, leng)
							.equals(IPFActionName.RECALL);
		} else {
			return actionVo.getAction_type() == BillActionTypeEnum.CancelSubmitWorkflow
					.getIntValue();
		}
	}
	...
	public static boolean isUnSaveAction(String actionName, String billtype) {
		...
		if (actionVo == null || actionVo.getAction_type() == null) {
			...
			return actionName.length() >= leng
					&& actionName.toUpperCase().substring(0, leng)
							.equals(IPFActionName.UNSAVE);
		} else {
			return actionVo.getAction_type() == BillActionTypeEnum.CancelSubmitApproveflow
					.getIntValue();
		}
	}
	...
	public static boolean isSignalFlowAction(String actionName, String billtype) {
		...
		return isSignalAction(actionName, billtype)
				|| isApproveAction(actionName, billtype);
	}
	...
	public static boolean isStartFlowAction(String actionName, String billtype) {
		...
		return isStartAction(actionName, billtype)
				|| isSaveAction(actionName, billtype);
	}
