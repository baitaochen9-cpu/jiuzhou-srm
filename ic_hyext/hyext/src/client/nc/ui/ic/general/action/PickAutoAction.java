...
public class PickAutoAction extends NCAction {
	...
	public void doAction(ActionEvent e) throws Exception {
		this.getEditorModel().getICBizView().getBillCardPanel().stopEditing();
		if (!this.onCheck()) {
			return;
		}
...
//	      ********************************行业添加 ***************************
	      Set<String> medLotNoSet = new HashSet<String>();// 存储批号id
			...
			if (medLotNoSet != null && !medLotNoSet.isEmpty()) {
				medlotList = (List<MedLotno_148>) queryService.queryBillOfVOByPKs(MedLotno_148.class,
						medLotNoSet.toArray(new String[medLotNoSet.size()]), false);
			...
			if (medBatchSet != null && !medBatchSet.isEmpty()) {
				...
				batcodeList = (List<BatchcodeVO>) queryService.queryBillOfVOByPKs(BatchcodeVO.class,
						medBatchSet.toArray(new String[medBatchSet.size()]), false);
				...
				for (int i = 0; i < bodyEntitys.length; i++) {
					...
					if (pk_batchcode != null && batcodeMap.containsKey(pk_batchcode)) {
						...
						this.getEditorModel().getCardPanelWrapper().getBillCardPanel()
								.setBodyValueAt(batchcode.getDproducedate(), i, "dproducedate");
						this.getEditorModel().getCardPanelWrapper().getBillCardPanel()
								.setBodyValueAt(batchcode.getDvalidate(), i, "dvalidate");
						this.getEditorModel().getCardPanelWrapper().getBillCardPanel()
								.setBodyValueAt(batchcode.getAttributeValue("vinvaliddate_148"), i, "vinvaliddate_148");
					}
					if (vfree6 != null && medlotMap.containsKey(vfree6)) {
						MedLotno_148 medlotno = medlotMap.get(vfree6);
						this.getEditorModel().getCardPanelWrapper().getBillCardPanel()
								.setBodyValueAt(medlotno.getDprodate(), i, "dproducedate");
						this.getEditorModel().getCardPanelWrapper().getBillCardPanel().setBodyValueAt(medlotno.getDvaliduntil(), i, "dvalidate");
						this.getEditorModel().getCardPanelWrapper().getBillCardPanel()
						.setBodyValueAt(medlotno.getDprodate(), i, "dproducedate_148");
						this.getEditorModel().getCardPanelWrapper().getBillCardPanel()
						.setBodyValueAt(medlotno.getVlotno(), i, "vlotno_148");
						this.getEditorModel().getCardPanelWrapper().getBillCardPanel()
								.setBodyValueAt(medlotno.getVinvaliddate(), i, "vinvaliddate_148");
					}
				}
			}
//		      ********************************行业添加 ***************************
...
//		wangsyf add 2020-06-15 ***************************************************************************************
		for (int i = 0; i < locs.length - 1; i++) {
			if(!fixedFlag){
	    		nassistnum = locs[i].getNassistnum();
	    	}else{
	 ...
	 */
	 ...
	 private void processOnhandDimInfo(ICBillBodyVO bodyvo, int row,
		 ...
		 for (String field : OnhandDimVO.getDimContentFields()) {
			 ...
			 // 修复bug:产成品辅助属性-项目字段空值保护,避免拣货时覆盖界面原有值 ERP材料出库单明细行项目分配出错
			 Object cprodprojectid = bodyvo.getAttributeValue("cprodprojectid");
			 if (field.equals("cprodprojectid")) {
				 if (cprodprojectid == null || StringUtil.isSEmptyOrNull((String) cprodprojectid)) {
					 Logger.debug("PickAutoAction.processOnhandDimInfo - 拣货结果中cprodprojectid为空,跳过设置,保护界面原有项目分配值. row=" + row);
					 continue;
				 } else {
					 Logger.debug("PickAutoAction.processOnhandDimInfo - 拣货结果中cprodprojectid有值,正常设置. row=" + row + ", value=" + cprodprojectid);
				 }
			 }
