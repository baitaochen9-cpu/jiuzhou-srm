...
package nc.ui.ic.m4i.action.extend;
    ...
    try {
      ...
      for (int i = 0, loop = billvo.getBodys().length; i < loop; i++) {
        ...
        for (int k = bodys.length - 1; k >= 0; k--) {
          if (k == bodys.length - 1) {
        	//add by chenqk vfree6通过公式无法赋上值
			getEditorModel().getCardPanelWrapper().getBillModel().setValueAt(bodys[k].getVfree6(), table.getSelectedRow(), "vdef6_148");
			//end
            this.setOnhandDataToBodyForMed(bodys[k], table.getSelectedRow());
          ...
          this.setOnhandDataToBodyForMed(bodys[k], setrow);
      ...
      ICBizCardBodyEntity[] bodyEntirys = this.getEditorModel().getICUIBillEntity().getBodys();
      if(bodyEntirys!=null&&bodyEntirys.length>0){
    	  for(ICBizCardBodyEntity bodyEntiry : bodyEntirys){
    		 
    		  String vfree6 = bodyEntiry.getVfree6();
	    	  String pk_batchcode = (String)bodyEntiry.getAttributeValue("pk_batchcode");
	    	  if(vfree6!=null){
      ...
      for(int i=0;i<bodyEntirys.length;i++){
 		 String vfree6 = bodyEntirys[i].getVfree6();
 		 String pk_batchcode = (String)bodyEntirys[i].getAttributeValue("pk_batchcode");
...
/** 由于粘贴行会把复制行的报价数量带过来，导致浮动换算率时计算报价换算率错误*/
  ...
  *
  */
 private void setOnhandDataToBodyForMed(ICBillBodyVO bodyvo, int row) {
   if (bodyvo == null) {
     return;
   }
   CardPanelWrapper wrapper = this.getEditorModel().getCardPanelWrapper();
   ...
   wrapper.getBillModel().setValueAt(bodyvo.getCrowno(), row,
       ICPubMetaNameConst.CROWNO);
   this.processOnhandDimInfoForMed(bodyvo, row, wrapper);
   // 货位管理仓，货位信息单独处理
   this.resetBodyLocationID(bodyvo, row);
   this.processCcorrespondInfo(bodyvo, row, wrapper);
  ...
  private void processOnhandDimInfoForMed(ICBillBodyVO bodyvo, int row,
