 ...
 *
 */
public class WoStartAlertPlugin extends AbstractAlertPlugin {
	...
	public PreAlertObject executeTask(PreAlertContext context)throws BusinessException {
		...
		if (CollectionUtils.isNotEmpty(pks)) {
			...
			retObj.setReturnType(PreAlertReturnType.RETURNDATASOURCE);//RETURNDATASOURCE
			retObj.setBusiObj(AggWorkOrderVO.class);
...
//		Object pk_org = context.getPk_org(); //获取组织面板中所选组织
		...
		//根据统计方式构造条件
		if(statmanner.equals("targ")){//统计方式：目标开始时间
		     sqlStr.append(" and ( targ_start_time <= '").append(currDate).append("' ");
		     sqlStr.append(" and targ_start_time > '").append(currDate.getDateBefore(Integer.parseInt((String)alertDays))).append("' )");
		}
		else if(statmanner.equals("plan")){//统计方式：计划开始时间
			 sqlStr.append(" and ( plan_start_time <= '").append(currDate).append("' ");
			 sqlStr.append(" and plan_start_time > '").append(currDate.getDateBefore(Integer.parseInt((String)alertDays))).append("' )");
		}
		else if(statmanner.equals("torp")){//统计方式：目标开始时间或计划开始时间
			 sqlStr.append("and ( ");
			 sqlStr.append(" ( ").append(" targ_start_time <= '").append(currDate).append("' ");
			 sqlStr.append(" and targ_start_time > '").append(currDate.getDateBefore(Integer.parseInt((String)alertDays))).append("' )");
			 sqlStr.append(" or ");
			 sqlStr.append(" ( ").append(" plan_start_time <= '").append(currDate).append("' ");
			 sqlStr.append(" and plan_start_time > '").append(currDate.getDateBefore(Integer.parseInt((String)alertDays))).append("' )");
		     sqlStr.append(" )");
		}
		     //作废类型工单、待批审类型工单不预警
		     ...
		     sqlStr.append(" )");
		     //已经开始的工单不做预警
		     sqlStr.append(" and actu_start_time is null ");
	...
	protected String getTitle() {
		...
		return nc.vo.ml.NCLangRes4VoTransl.getNCLangRes().getStrByID("workorder_0","04560003-0515")/*@res "工单延期开始预警"*/;
	}
