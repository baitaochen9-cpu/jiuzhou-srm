 ...
 * @description
 * @author xiahui
 * @date 创建时间：2019-3-8 下午4:49:09
 * @version ncc1.0
 **/
public class IitermdaysBodyAfterEditRule implements ITableAfterRule<PayPlanViewVO> {
	...
	public PayPlanViewVO[] afterEdit(PayPlanViewVO[] vo, TableAfterEditEvent event, Map userObj) {
		...
		Object newValue = event.getChangedRows()[0].getNewvalue();
		if (null == newValue) {
			util.setViewValue(AbstractPayPlanVO.DBEGINDATE, null);
			return vo;
		}
		...
		int iitermdays = Integer.parseInt((String) newValue);
