 ...
 * @version ncc1.0
 */
public class SaleDailyExpCodeAfterRule implements
	...
	public AggCtSaleVO afterEdit(AggCtSaleVO billvo,
		...
		for(int i = 0;i<rows.length;i++) {
			...
			if (!ValueUtil.isEmpty(pk_org)) {
				billvo.getCtSaleExpVO()[i].setPk_org(pk_org);
			}
			if (!ValueUtil.isEmpty(pk_org_v)) {
				billvo.getCtSaleExpVO()[i].setPk_org_v(pk_org_v);
			}
			if (!ValueUtil.isEmpty(pk_group)) {
				billvo.getCtSaleExpVO()[i].setPk_group(pk_group);
			}
		}
