package nccloud.web.ct.purdaily.transtype;
...
/**
 * @description 交易类型面板查询
 * @author xiahui
 * @date 创建时间：2019-2-19 下午3:31:38
 * @version ncc1.0
 **/
public class QueryTransTypeAction implements ICommonAction {
	...
	public Object doAction(IRequest request) {
		try {
			...
			BusinessSetVO[] returnVos = BusinessTypeService.queryBusinesstypeExtProp(" and ctrantypeid = '" + ctrantypeid
					+ "' and dr = 0 ");

			if (returnVos == null || returnVos.length == 0) {
				return null;
			}
			TransTypeConvertor convertor = new TransTypeConvertor();
