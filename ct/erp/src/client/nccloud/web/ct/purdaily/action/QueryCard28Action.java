...
import nccloud.web.ct.purdaily.utils.Ref28TransferUtil;

/**
 * @description 采购合同维护拉价格审批单转换
 * @author xiahui
 * @date 创建时间：2019-2-18 上午10:53:47
 * @version ncc1.0
 **/
public class QueryCard28Action implements ICommonAction {

	@Override
	public Object doAction(IRequest request) {
		ExtBillCard[] cards = null;
		try {
			Ref28TransferUtil transferUtils = new Ref28TransferUtil();
			AggCtPuVO[] vos = transferUtils.getAggCtPuVO(request);

			ExtBillCardOperator operator = new ExtBillCardOperator(
					transferUtils.getTransferInfo(request).getPagecode());
