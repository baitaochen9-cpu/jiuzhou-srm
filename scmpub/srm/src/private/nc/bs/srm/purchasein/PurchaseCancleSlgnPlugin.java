 ...
 *
 */
public class PurchaseCancleSlgnPlugin {
	...
	public void process(String billltypecode, Object obj, Map<String, Object> otherpms) throws BusinessException {
		AbstractSender4Mdm sender = new PurchaseSlgnSender();
		...
		try {
			...
			if (resp != null) {
				...
				if (!"ERROR".equals(responseStatus)) {
					...
					throw new BusinessException("srm拒收." + ERROR);
				}
			} else {
				throw new BusinessException("采购入库取消签字推送srm返回结果为空.");
			}
			// 更新日志
		} catch (Exception e) {
			Logger.error(e.getMessage(), e);
			throw new BusinessException("采购入库取消签字推送srm同步失败:" + e.getMessage());
		}
