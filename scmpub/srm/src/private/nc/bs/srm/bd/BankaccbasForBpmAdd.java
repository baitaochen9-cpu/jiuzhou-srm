 ...
 * 
 */
public class BankaccbasForBpmAdd  {
	public Object processBill(BankAccbasVO resvo,SupplierVO supVO) throws BusinessException {
		// 校验导入数据的合法性
		checkData(resvo);
		// 3.补全数据,并且调整单据状态
		fillData(resvo);
		...
		//根据账号查询是否存在
		BankAccbasVO queryBankaccInfor = queryBankaccInfor(unitvo,supVO);
		
		if(queryBankaccInfor !=null){
			BankAccSubVO[] bankaccsubs = queryBankaccInfor.getBankaccsub();
			if(bankaccsubs!=null) {
				for(BankAccSubVO bankaccsubvo:bankaccsubs) {
				}
			}
		...
		if (queryBankaccInfor ==null) {
			setVOStatus( resvo, VOStatus.NEW);
			bankvo = service.insertCustBankacc(unitvo);
			vopk = bankvo.getPrimaryKey();
			//先启用
			resvo.setEnablestate(2);
			if(resvo.getEnableuser() !=null){
				InvocationInfoProxy.getInstance().setUserId(resvo.getEnableuser());
			}
//			service.enableCustBankacc(bankvo);
	...
	public CustBankaccUnionVO getCustBankaccUnionVO(BankAccbasVO bankvo)
			throws BusinessException {
		// TODO 自动生成的方法存根
		// TODO 自动生成的方法存根
		bankvo.setEnablestate(2);
	...
	private CustbankVO getCustBankVO(BankAccbasVO bankvo, String pk_cust) {
		...
		cbvo.setIsdefault(UFBoolean.TRUE);
	...
	private void setVOStatus( BankAccbasVO resvo,
	 ...
	 * @throws BusinessException
	 */
	public BankAccbasVO queryBankaccInfor(CustBankaccUnionVO unionVO,SupplierVO supVO)
			throws BusinessException {
		...
		int accclass = 3;
		HashMap<String,BankAccbasVO> accMap = accMap();
		CustbankVO[] supbankacc = supVO.getSupbankacc();
		...
		//使用供应商自带的银行张辉作比较 先用自带的做修改
		if(supbankacc != null && supbankacc.length >0){
			for (CustbankVO custbankVO : supbankacc) {
				String pk_bankaccbas = custbankVO.getPk_bankaccbas();
				BankAccbasVO bankAccbasVO2 = accMap.get(pk_bankaccbas);
				if(bankAccbasVO2 == null){
					continue;
				}
				if(accnum.equals(bankAccbasVO2.getAccnum())){
					return bankAccbasVO2;
				}
			}
		}
		...
		if (col==null || col.size()==0) {
			//解决之前的错误数据
			String where1 = " accnum = '" + accnum + "'";
			Collection<BankAccbasVO> coll = getQuerySer().retrieveByClause(
					BankAccbasVO.class, where1);
			if(coll == null || coll.size() == 0){
				return null;
			}
			return coll.toArray(new BankAccbasVO[0])[0];
		}
	...
	private List<BankAccbasVO> getAccBas() throws BusinessException{
		List<BankAccbasVO> voList = (List<BankAccbasVO>)getQuerySer().retrieveByClause(BankAccbasVO.class, "dr = 0 and enablestate =2");
		return voList;
	}
	private HashMap<String,BankAccbasVO> accMap() throws BusinessException{
		List<BankAccbasVO> accBas = getAccBas();
		HashMap<String,BankAccbasVO> map = new HashMap<>();
		for (BankAccbasVO bankAccbasVO : accBas) {
			map.put(bankAccbasVO.getPk_bankaccbas(), bankAccbasVO);
		}
		return map;
	}
