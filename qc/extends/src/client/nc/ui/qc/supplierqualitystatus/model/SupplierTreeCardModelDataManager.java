...
import nc.ui.mmgp.uif2.model.MMGPBaseModelDataManager;
 ...
 * 
 */
public class SupplierTreeCardModelDataManager extends MMGPBaseModelDataManager implements AppEventListener {
                ...
                if (bean.getTagString() != null && bean.getTagString().contains("BDMODE")) {
                    orgSql =
                            VisibleUtil.getRefVisibleCondition(
                                context.getPk_group(),
                                pk_orgs,
//                                MMMetaUtils.getBeanByClassFullName(className).getID()
...
//                                    " pk_org in (" +context.getOrgVO().getPk_org() + ")",
...
//                        orgSql = conditionBuilder.getCondition(bean.getTable().getName());
				...
				if(o != null){
					getTreeModel().setSelectedData(o);
				}
//			} else {
