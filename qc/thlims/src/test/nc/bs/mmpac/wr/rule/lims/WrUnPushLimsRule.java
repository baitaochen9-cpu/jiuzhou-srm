...
public class WrUnPushLimsRule implements IRule<AggWrVO> {
	@Override
	public void process(AggWrVO[] vos) {
		...
		try {
			...
			for (AggWrVO aggWrVO : vos) {
				...
				WrItemVO[] bodys =(WrItemVO[]) aggWrVO.getChildren(WrItemVO.class);
				for (WrItemVO itemVO :bodys ) {					
//					fbproducttype  产品类型  fbproducttype int  产出类型   1=主产品，2=联产品，3=副产品，   
				...
				sysdisptch.dispatch(aggWrVO, "LIMS_WR_CHECK_CANCEL", param);
			}
		} catch (Exception e) {
