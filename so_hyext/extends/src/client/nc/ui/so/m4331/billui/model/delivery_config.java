 ...
 * 外部COA按钮
 * */
public nc.ui.so.m4331.billui.action.ThLimsCoaAction getThLimsCoaAction(){
	 if(context.get("ThLimsCoaAction")!=null)
	 return (nc.ui.so.m4331.billui.action.ThLimsCoaAction)context.get("ThLimsCoaAction");
	 nc.ui.so.m4331.billui.action.ThLimsCoaAction bean = new nc.ui.so.m4331.billui.action.ThLimsCoaAction();
	  context.put("thLimsCoaAction",bean);
	  bean.setModel(getManageAppModel());
	  bean.setEditor(getBillFormEditor());
	  bean.setListView(getListView());
	setBeanFacotryIfBeanFacatoryAware(bean);
